...
export function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-60 bg-gray-950 border-r border-gray-800 flex flex-col z-40">
      {/* Logo */}
      <div className="h-16 flex items-center px-5 border-b border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold tracking-wide text-white">BRAHMASTRA</h1>
            <p className="text-[10px] text-gray-500 tracking-widest uppercase">Trading System</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <span className={clsx(isActive ? "text-accent" : "text-gray-500")}>
                {item.icon}
              </span>
              {item.label}
              {isActive && (
