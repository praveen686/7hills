...
pub use quantlaxmi_runner_common::{
    IntegritySummary, RunnerConfig, SessionManifest, TickOutputEntry, UnderlyingEntry,
...
pub enum Commands {
    ...
    /// Capture multi-instrument session with TickEvent JSONL.
    ///
    ...
    /// Run paper trading mode (headless only, TUI not yet implemented)
    Paper {
...
async fn async_main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Paper {
            initial_capital,
            duration_secs,
        } => run_paper_mode(initial_capital, duration_secs).await,
        Commands::Live {
            config,
            initial_capital,
...
async fn run_paper_mode(initial_capital: f64, duration_secs: u64) -> anyhow::Result<()> {
    info!("QuantLaxmi India - Paper Trading Mode");
    tui::print_headless_banner("QuantLaxmi India Paper Trading", initial_capital);

    let config = paper::IndiaPaperConfig {
        initial_capital,
        duration_secs,
        ..Default::default()
    };

    paper::run(&config).await
}
