...
impl MicroAlpha {
    fn new(window: usize) -> Self {
        Self {
            imbalance_buffer: VecDeque::with_capacity(window),
            window,
        }
    }
    ...
    fn update_book(&mut self, bid_qty: f64, ask_qty: f64) {
        ...
        if self.imbalance_buffer.len() > self.window {
            self.imbalance_buffer.pop_front();
        }
    }
}
...
impl AlphaSource for MicroAlpha {
    fn get_signal(&self) -> f64 {
        if self.imbalance_buffer.is_empty() {
            return 0.0;
        }
...
impl RegimeClassifier {
    ...
    fn classify(&self) -> MarketRegime {
        ...
        let std_ret = (returns.iter().map(|r| (r - mean_ret).powi(2)).sum::<f64>()
            / returns.len() as f64)
            .sqrt();
        ...
        let range = prices.iter().fold(f64::MIN, |a, &b| a.max(b))
            - prices.iter().fold(f64::MAX, |a, &b| a.min(b));
...
impl OrasStrategy {
    ...
    fn self_correct(&mut self) {
        ...
        let avg_slippage = if self.slippage_buffer.is_empty() {
            0.0
        } else {
            self.slippage_buffer.iter().sum::<f64>() / self.slippage_buffer.len() as f64
        };

        if avg_slippage > 0.0002 {
            // 2 bps
            info!(
                "[ORAS] High slippage detected ({:.2} bps). Recalibrating weights...",
                avg_slippage * 10000.0
            );
            self.w_micro *= 0.9;
...
impl crate::Strategy for OrasStrategy {
    ...
    fn on_fill(&mut self, fill: &OrderEvent) {
        if let OrderPayload::Update { .. } = &fill.payload {
            ...
            if self.slippage_buffer.len() > 10 {
                self.slippage_buffer.pop_front();
            }
            self.self_correct();
        }
    }
    ...
    fn on_stop(&mut self) {
        info!(
            "[ORAS] Strategy Cycle Terminated. Final Position: {}",
            self.position
        );
    }
...
mod tests {
    ...
    fn test_regime_classification_basic() {
        ...
        for i in 0..15 {
            ...
            if classifier.prices.len() > 10 {
                classifier.prices.pop_front();
            }
        }
        ...
        assert!(matches!(
            regime,
            MarketRegime::Trending | MarketRegime::Idle
        ));
    }
}
