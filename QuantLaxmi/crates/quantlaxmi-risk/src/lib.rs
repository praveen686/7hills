    ...
    pub fn check_order(&self, event: &OrderEvent, current_price: f64) -> Result<(), RiskViolation> {
        ...
        } = &event.payload
        {
            ...
            if order_value > self.config.max_order_value_usd {
                ...
                metrics::counter!("quantlaxmi_risk_checks_total", "result" => "failed", "reason" => "order_value").increment(1);
            ...
            if new_notional > self.config.max_notional_per_symbol_usd {
                ...
                metrics::counter!("quantlaxmi_risk_checks_total", "result" => "failed", "reason" => "max_notional").increment(1);
        ...
        metrics::counter!("quantlaxmi_risk_checks_total", "result" => "passed").increment(1);
        Ok(())
    }
...
mod tests {
    use super::*;
    use chrono::Utc;
    use quantlaxmi_models::OrderType;
    use uuid::Uuid;
