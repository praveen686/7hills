...
impl OptionsMargin {
    ...
    pub fn for_option(
        ...
        let intrinsic_up = if is_call {
            (stress_up - strike).max(0.0)
        } else {
            (strike - stress_up).max(0.0)
        };
        let intrinsic_down = if is_call {
            (stress_down - strike).max(0.0)
        } else {
            (strike - stress_down).max(0.0)
        };

        // SPAN margin = worst case loss
        let span = if is_short {
            let worst_loss =
                intrinsic_up.max(intrinsic_down) * quantity.abs() as f64 * lot_size as f64;
    ...
    pub fn check_limits(&self, max_delta: f64, max_gamma: f64, max_vega: f64) -> Vec<String> {
        ...
        if self.total_delta.abs() > max_delta {
            violations.push(format!(
                "Delta limit exceeded: {:.2} > {:.2}",
                self.total_delta.abs(),
                max_delta
            ));
        }
        ...
        if self.total_gamma.abs() > max_gamma {
            violations.push(format!(
                "Gamma limit exceeded: {:.4} > {:.4}",
                self.total_gamma.abs(),
                max_gamma
            ));
        }
...
impl OptionsVaR {
    ...
    pub fn calculate(
        ...
        let gamma_var =
            0.5 * portfolio_greeks.total_gamma.abs() * price_move.powi(2) * lot_size as f64;
    ...
    pub fn historical_var(
        ...
        let mut pnls: Vec<f64> = historical_returns
            .iter()
            .map(|ret| {
                let price_change = spot_price * ret;
                let delta_pnl = portfolio_greeks.total_delta * price_change * lot_size as f64;
                let gamma_pnl =
                    0.5 * portfolio_greeks.total_gamma * price_change.powi(2) * lot_size as f64;
                delta_pnl + gamma_pnl
            })
            .collect();
...
mod tests {
    ...
    fn test_short_option_margin() {
        ...
        assert!(
            margin.total_margin > 0.0,
            "Short option should require margin"
        );
    ...
    fn test_long_option_no_margin() {
        let margin = OptionsMargin::for_option(
            25800.0, 25800.0, true, false, // long
            1, 50, 0.15,
        );

        assert_eq!(
            margin.span_margin, 0.0,
            "Long option should have no SPAN margin"
        );
        assert_eq!(
            margin.total_margin, 0.0,
            "Long option should have no margin requirement"
        );
    }
    ...
    fn test_portfolio_delta_limit() {
        ...
        assert!(
            !violations.is_empty(),
            "Should detect delta limit violation"
        );
    }
    ...
    fn test_historical_var() {
        ...
        let returns = vec![
            -0.02, -0.01, 0.005, 0.01, -0.015, 0.02, -0.025, 0.003, -0.008, 0.012,
        ];
