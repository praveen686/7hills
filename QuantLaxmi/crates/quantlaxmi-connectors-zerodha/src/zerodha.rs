...
impl ZerodhaAutoDiscovery {
    ...
    /// Discover symbols for next week's expiry (for calendar spreads).
    ///
    /// Finds the actual front expiry from available instruments first (not relying
    /// on a hardcoded weekday), then selects the next expiry strictly after it.
    pub async fn discover_next_week_symbols(
        &self,
        config: &AutoDiscoveryConfig,
    ) -> anyhow::Result<Vec<(String, u32)>> {
        // 1. Get current spot price
        ...
        // 3. Find the actual front expiry (nearest expiry >= today) — don't assume Thursday
        let today = Utc::now().date_naive();
        let mut all_expiries: Vec<NaiveDate> = instruments
            .iter()
            .filter(|i| i.name == config.underlying)
            .filter(|i| i.expiry >= today)
            .map(|i| i.expiry)
            .collect::<std::collections::HashSet<_>>()
            .into_iter()
            .collect();
        ...
        if all_expiries.len() < 2 {
            return Err(anyhow::anyhow!(
                "Need at least 2 expiries for calendar spread, found {}",
                all_expiries.len()
            ));
        }

        let front_expiry = all_expiries[0];
        let back_expiry = all_expiries[1];

        info!(
            "[AUTO-DISCOVERY] Calendar spread: front={}, back={} ({} expiries available)",
            front_expiry,
            back_expiry,
            all_expiries.len()
        );

        self.discover_symbols_for_expiry(config, spot, atm, back_expiry, &instruments)
            .await
    }
