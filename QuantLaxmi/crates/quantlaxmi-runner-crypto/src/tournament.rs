...
pub struct GridRunResult {
    ...
    /// GPU batch score (if GPU scoring enabled)
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub async fn run_tournament_grid_cli(
    ...
    max_runs: usize,
    parallel: usize,
    two_pass_config: Option<crate::two_pass::TwoPassConfig>,
    pass2_only: bool,
    pass1_dir: Option<&str>,
    shard_config: Option<crate::shard::ShardConfig>,
) -> Result<()> {
    use rayon::prelude::*;
    use std::sync::Mutex;
    ...
    if let Some(ref sc) = shard_config {
        info!("Shard: {}/{}", sc.shard_index, sc.shard_count);
    }
    ...
    // Discover segments
    let all_segments = discover_segments(&segments_root_path, &patterns)?;
    info!("Matched {} segments", all_segments.len());

    // Parse and expand grid
    let all_param_sets = parse_and_expand_grid(&grid_path_buf)?;
    info!(
        "Grid expanded to {} parameter combinations",
        all_param_sets.len()
    );

    // === Two-Pass Mode Handling ===
    let (segments, param_sets, is_pass2) = if let Some(ref tp_config) = two_pass_config {
        if pass2_only {
            // Pass 2 only: load selected configs from Pass 1
            let p1_dir = pass1_dir
                .map(PathBuf::from)
                .ok_or_else(|| anyhow::anyhow!("--pass1-dir required for --pass2-only"))?;

            let selected = crate::two_pass::load_selected_configs(&p1_dir)?;
            info!(
                "Pass 2: loaded {} selected configs from Pass 1",
                selected.len()
            );

            // Filter param_sets to only those selected
            let selected_hashes: std::collections::HashSet<String> =
                selected.iter().map(|c| c.param_hash.clone()).collect();

            let filtered_params: Vec<BTreeMap<String, ParamValue>> = all_param_sets
                .into_iter()
                .filter(|p| {
                    let hash = compute_param_hash(p);
                    selected_hashes.contains(&hash)
                })
                .collect();

            info!(
                "Pass 2: running {} configs on all {} segments",
                filtered_params.len(),
                all_segments.len()
            );
            (all_segments, filtered_params, true)
        } else {
            // Pass 1: stratified segment selection
            let segment_names: Vec<String> = all_segments
                .iter()
                .map(|p| {
                    p.file_name()
            ...
            // Build seed material from strategy + grid hash
            let grid_content = std::fs::read(&grid_path_buf)?;
            let grid_hash = hex::encode(Sha256::digest(&grid_content));
            let seed_material = format!("{}:{}", strategy, grid_hash);

            let selection =
                crate::two_pass::select_pass1_segments(&segment_names, tp_config, &seed_material);
            info!(
                "Pass 1: selected {} of {} segments (bins={}, seed={})",
                selection.selected_count,
                selection.total_segments,
                selection.bins_used,
                &selection.seed_hex[..8]
            );

            // Filter segments to selected ones
            let selected_segments: Vec<PathBuf> = selection
                .selected_indices
                .iter()
                .map(|&i| all_segments[i].clone())
                .collect();

            // Store selection for later artifact writing
            // (We'll write it after the run completes)

            (selected_segments, all_param_sets, false)
        }
    } else {
        // Standard single-pass mode
        (all_segments, all_param_sets, false)
    };

    // Safety check: max_runs (checked against full grid, before sharding)
    let full_grid_runs = segments.len() * param_sets.len();
    if full_grid_runs > max_runs {
        return Err(anyhow::anyhow!(
    ...
    info!(
        "Total grid runs: {} ({} will be executed per shard)",
        full_grid_runs,
    ...
    // === Sharding: Filter tasks to this shard ===
    let (run_tasks, shard_manifest_builder) = if let Some(ref sc) = shard_config {
        // Build simplified task list for filtering
        let task_list: Vec<(String, String, RunTask)> = run_tasks
            .into_iter()
            .map(|t| (t.1.clone(), t.2.clone(), t)) // (segment_name, param_hash, task)
            .collect();

        let (filtered, task_list_hash, assigned_keys) =
            crate::shard::filter_tasks_for_shard(&task_list, sc);
        let filtered_tasks: Vec<RunTask> = filtered.into_iter().map(|(_, _, t)| t).collect();

        info!(
            "Shard {}/{}: {} of {} tasks assigned",
            sc.shard_index,
            sc.shard_count,
            filtered_tasks.len(),
            task_list.len()
        ...
        // Create manifest builder for tracking completions
        let builder = crate::shard::ShardManifestBuilder::new(
            sc.clone(),
            task_list_hash,
            strategy,
            &grid_hash,
        ...
        (filtered_tasks, Some(std::sync::Mutex::new(builder)))
    } else {
        (run_tasks, None)
    };

    // Update total_runs to reflect actual tasks after sharding
    let total_runs = run_tasks.len();
                ...
                match result {
                    Ok((metrics, manifest, diagnostics, summary)) => {
                        ...
                        let grid_result = GridRunResult {
                            ...
                            diagnostics,
                            summary,
                            gpu_score: None, // Set later by GPU batch scoring
                        };
                        results_mutex.lock().unwrap().push(grid_result);

                        // Record completion for shard manifest
                        if let Some(ref builder_mutex) = shard_manifest_builder {
                            builder_mutex
                                .lock()
                                .unwrap()
                                .record_completion(segment_name, param_hash);
                        }
                    }
                    Err(e) => {
    ...
    // --- GPU batch scoring wedge (B) ---
    // Score all runs in a single GPU batch for ranking.
    // This is optional: if GPU scoring fails, we fall back to CPU composite score.
    let gpu_scoring_enabled = std::env::var("QUANTLAXMI_GPU_SCORING")
        .map(|v| v == "1" || v.to_lowercase() == "true")
        .unwrap_or(false);

    if gpu_scoring_enabled && !results.is_empty() {
        info!(
            "GPU batch scoring enabled, scoring {} runs...",
            results.len()
        );
        let gpu_work_dir = out_dir_path.join("gpu_scoring");

        // Build FeatureRows aligned 1:1 with results ordering
        let feature_rows: Vec<crate::gpu_batch::FeatureRow> = results
            .iter()
            .map(|r| {
                // Refusal predicate: no-trade runs or invalid metrics
                let refuse = r.metrics.total_trades == 0
                    || crate::gpu_batch::has_invalid_metrics(&r.metrics);

                crate::gpu_batch::FeatureRow {
                    refuse,
                    x: crate::gpu_batch::metrics_to_feature_vec(&r.metrics),
                    config_hash_hex: r.param_hash.clone(),
                }
            })
            .collect();

        match crate::gpu_batch::score_rows_with_gpu(&gpu_work_dir, &feature_rows) {
            Ok(scored) => {
                anyhow::ensure!(
                    scored.len() == results.len(),
                    "GPU scores misaligned: {} vs {}",
                    scored.len(),
                    results.len()
                );
                // Attach GPU scores to results
                for (r, s) in results.iter_mut().zip(scored.iter()) {
                    r.gpu_score = Some(s.score);
                }
                info!("GPU batch scoring complete");

                // Write score parity report (CPU vs GPU comparison)
                write_score_parity_report(&out_dir_path, &results)?;
            }
            Err(e) => {
                warn!("GPU scoring failed, falling back to CPU: {}", e);
                // Leave gpu_score as None, build_grid_leaderboard will use CPU composite
            }
        }
    }

    // === Two-Pass Mode: Pass 1 Aggregation and Selection ===
    if let Some(ref tp_config) = two_pass_config
        && !is_pass2
    {
        // Pass 1 complete - aggregate and select configs for Pass 2
        info!("Pass 1 aggregation: computing robust stats per config...");

        // Rebuild segment selection for artifact writing
        let _segment_names_for_selection: Vec<String> = segments
            .iter()
            .map(|p| {
                p.file_name()
                    .map(|n| n.to_string_lossy().to_string())
        ...
        let grid_content = std::fs::read(&grid_path_buf)?;
        let grid_hash_for_seed = hex::encode(Sha256::digest(&grid_content));
        let seed_material = format!("{}:{}", strategy, grid_hash_for_seed);

        // Get all segment names (not just selected) for the selection artifact
        let all_segment_names: Vec<String> = discover_segments(&segments_root_path, &patterns)?
            .iter()
            .map(|p| {
                p.file_name()
                    .map(|n| n.to_string_lossy().to_string())
        ...
        let selection =
            crate::two_pass::select_pass1_segments(&all_segment_names, tp_config, &seed_material);

        // Aggregate results
        let mut aggregations = crate::two_pass::aggregate_pass1_results(&results, tp_config);

        // Select top-K for Pass 2
        let selected_configs =
            crate::two_pass::select_configs_for_pass2(&mut aggregations, tp_config);

        // Write Pass 1 artifacts
        crate::two_pass::write_pass1_artifacts(
            &out_dir_path,
            &selection,
            &aggregations,
            &selected_configs,
        ...
        info!(
            "Pass 1 complete. Run Pass 2 with: --pass2-only --pass1-dir {}",
    ...
    // Compute stability metrics if this is Pass 2 (or standard single-pass)
    if is_pass2 || two_pass_config.is_none() {
        info!("Computing stability metrics...");
        let stability = crate::two_pass::compute_pass2_stability(&results, 10);
        let stability_stats = crate::two_pass::compute_pass2_stats(&stability, segments.len());
        crate::two_pass::write_pass2_stability(&out_dir_path, &stability, &stability_stats)?;
    }
    ...
    // === Write shard manifest v2 if sharding enabled ===
    if let Some(builder_mutex) = shard_manifest_builder {
        let builder = builder_mutex.into_inner().unwrap();
        let shard_manifest = builder.build(&results_path)?;

        info!(
            "Shard {}/{}: {}/{} tasks completed, {} failed",
            shard_manifest.shard_index,
            shard_manifest.shard_count,
            shard_manifest.completed_runs,
            shard_manifest.total_tasks_assigned,
            shard_manifest.failed_runs
        );

        crate::shard::write_shard_manifest(&out_dir_path, &shard_manifest)?;
    }
    ...
    if shard_config.is_some() {
        info!("  Mode: Sharded (merge with --merge-shards)");
    }
...
/// Write CPU vs GPU score parity report.
///
/// Produces:
/// - score_parity.csv with per-config comparison
/// - score_parity_stats.json with summary statistics
fn write_score_parity_report(out_dir: &Path, results: &[GridRunResult]) -> Result<()> {
    use std::io::Write;

    // Compute CPU scores for all results (same formula as build_grid_leaderboard fallback)
    let cpu_scores: Vec<f64> = results
        .iter()
        .map(|r| {
            let return_pct = r.manifest.return_pct / 100.0;
            let dd_pct = r.metrics.max_drawdown_pct;
            return_pct - 2.0 * dd_pct
        })
        .collect();

    // Build CSV
    let csv_path = out_dir.join("score_parity.csv");
    let file = std::fs::File::create(&csv_path)?;
    let mut writer = std::io::BufWriter::new(file);

    writeln!(
        writer,
        "config_hash,segment,cpu_score,gpu_score,delta,refuse"
    )?;

    for (i, r) in results.iter().enumerate() {
        let cpu = cpu_scores[i];
        let gpu = r.gpu_score.unwrap_or(f32::NAN);
        let delta = if gpu.is_finite() {
            gpu as f64 - cpu
        } else {
            f64::NAN
        };
        let refuse = if r.metrics.total_trades == 0 { 1 } else { 0 };

        writeln!(
            writer,
            "{},{},{:.6},{:.6},{:.6},{}",
            r.param_hash, r.segment, cpu, gpu, delta, refuse
        )?;
    }
    drop(writer);
    ...
    // Only for non-refused, valid GPU scores
    let valid_pairs: Vec<(f64, f64)> = results
        .iter()
        .zip(cpu_scores.iter())
        .filter(|(r, _)| {
            r.gpu_score.is_some() && r.gpu_score.unwrap() > -1e20 && r.metrics.total_trades > 0
        })
        .map(|(r, &cpu)| (cpu, r.gpu_score.unwrap() as f64))
        .collect();

    let (spearman_corr, top_10_overlap, top_50_overlap) = if valid_pairs.len() >= 10 {
        // Compute ranks
        let mut cpu_with_idx: Vec<(usize, f64)> = valid_pairs
            .iter()
            .enumerate()
            .map(|(i, (c, _))| (i, *c))
            .collect();
        let mut gpu_with_idx: Vec<(usize, f64)> = valid_pairs
            .iter()
            .enumerate()
            .map(|(i, (_, g))| (i, *g))
            .collect();

        cpu_with_idx.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
        gpu_with_idx.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));

        let mut cpu_ranks = vec![0usize; valid_pairs.len()];
        let mut gpu_ranks = vec![0usize; valid_pairs.len()];

        for (rank, (idx, _)) in cpu_with_idx.iter().enumerate() {
            cpu_ranks[*idx] = rank;
        }
        for (rank, (idx, _)) in gpu_with_idx.iter().enumerate() {
            gpu_ranks[*idx] = rank;
        }

        // Spearman correlation: 1 - (6 * sum(d^2)) / (n * (n^2 - 1))
        let n = valid_pairs.len() as f64;
        let sum_d_sq: f64 = cpu_ranks
            .iter()
            .zip(gpu_ranks.iter())
            .map(|(&c, &g)| {
            ...
            .sum();
        let spearman = 1.0 - (6.0 * sum_d_sq) / (n * (n * n - 1.0));

        // Top-K overlap
        let top_10_cpu: std::collections::HashSet<usize> =
            cpu_with_idx.iter().take(10).map(|(i, _)| *i).collect();
        let top_10_gpu: std::collections::HashSet<usize> =
            gpu_with_idx.iter().take(10).map(|(i, _)| *i).collect();
        let top_10_overlap = top_10_cpu.intersection(&top_10_gpu).count() as f64 / 10.0;

        let top_50_cpu: std::collections::HashSet<usize> = cpu_with_idx
            .iter()
            .take(50.min(valid_pairs.len()))
            .map(|(i, _)| *i)
            .collect();
        let top_50_gpu: std::collections::HashSet<usize> = gpu_with_idx
            .iter()
            .take(50.min(valid_pairs.len()))
            .map(|(i, _)| *i)
            .collect();
        let top_50_overlap = top_50_cpu.intersection(&top_50_gpu).count() as f64
            / 50.0_f64.min(valid_pairs.len() as f64);

        (spearman, top_10_overlap, top_50_overlap)
    } else {
    ...
    let stats = serde_json::json!({
        ...
        "valid_gpu_scores": valid_pairs.len(),
        "refused_count": results.iter().filter(|r| r.metrics.total_trades == 0).count(),
        "spearman_rank_correlation": spearman_corr,
        "top_10_overlap_pct": top_10_overlap * 100.0,
        "top_50_overlap_pct": top_50_overlap * 100.0,
        "cpu_score_range": {
            "min": cpu_scores.iter().cloned().filter(|x| x.is_finite()).fold(f64::INFINITY, f64::min),
            "max": cpu_scores.iter().cloned().filter(|x| x.is_finite()).fold(f64::NEG_INFINITY, f64::max),
        },
        "gpu_score_range": {
            "min": results.iter().filter_map(|r| r.gpu_score).filter(|&s| s > -1e20).fold(f32::INFINITY, f32::min),
            "max": results.iter().filter_map(|r| r.gpu_score).filter(|&s| s > -1e20).fold(f32::NEG_INFINITY, f32::max),
        },
    });

    let stats_path = out_dir.join("score_parity_stats.json");
    std::fs::write(&stats_path, serde_json::to_string_pretty(&stats)?)?;
    info!("Wrote {}", stats_path.display());
    info!(
        "Score parity: Spearman={:.3}, Top-10 overlap={:.0}%, Top-50 overlap={:.0}%",
        spearman_corr,
        top_10_overlap * 100.0,
        top_50_overlap * 100.0
    );
...
pub fn build_grid_leaderboard(results: &[GridRunResult]) -> Vec<LeaderboardEntry> {
    // Group by param_hash
    ...
    for (param_hash, runs) in &by_hash {
        ...
        // Score: use GPU scores if available, otherwise fall back to CPU composite
        let gpu_scores: Vec<f32> = runs
            .iter()
            .filter_map(|r| r.gpu_score)
            .filter(|&s| s > -1e20) // Exclude refused sentinel scores
            .collect();

        let score = if !gpu_scores.is_empty() {
            // Mean GPU score across segments (authoritative when present)
            (gpu_scores.iter().sum::<f32>() / gpu_scores.len() as f32) as f64
        } else {
            // Fallback: CPU composite score (v0)
            mean_return - 2.0 * mean_drawdown
        };
...
pub fn select_promotion_candidates(
