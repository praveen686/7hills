...
pub async fn run_tournament_grid_cli(
    ...
    // Build list of all run tasks: (segment_path, segment_name, param_hash, params, config_path)
    ...
    pool.install(|| {
        run_tasks.par_iter().for_each(
            |(segment_path, segment_name, param_hash, params, config_path)| {
                ...
                // Collect result
                match result {
                    Ok((metrics, manifest, diagnostics)) => {
...
async fn run_single_grid_backtest(
    ...
    Ok((metrics_v1, manifest_v1, diagnostics))
}
