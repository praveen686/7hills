...
// ---------- Replay (Time-Travel Playback) ----------

export async function fetchReplayDates(): Promise<string[]> {
  return apiFetch<string[]>("/api/replay/dates");
}

export async function fetchReplayTimeline(date: string): Promise<TimelineMarker[]> {
  return apiFetch<TimelineMarker[]>(`/api/replay/timeline/${date}`);
}
...
export async function fetchReplaySnapshot(
  ...
  return apiFetch<ReplaySnapshot>(
    `/api/replay/snapshot/${encodeURIComponent(ts)}?date=${date}`
  );
}

export async function fetchReplayStep(
  fromTs: string,
  deltaMs: number,
  date: string
): Promise<StepResult> {
  const params = new URLSearchParams({
    from_ts: fromTs,
    delta_ms: String(deltaMs),
    date,
  });
  return apiFetch<StepResult>(`/api/replay/step?${params.toString()}`);
}

// ---------- Diagnostics ----------

export async function fetchTradeAnalytics(
  strategyId?: string,
  limit = 200,
  offset = 0,
  ...
  return apiFetch<TradeAnalytics[]>(
    `/api/diagnostics/trades/analytics?${params.toString()}`
  );
}

export async function fetchTradeAnalyticsSummary(): Promise<TradeAnalyticsSummary> {
  return apiFetch<TradeAnalyticsSummary>("/api/diagnostics/trades/summary");
}
...
export async function fetchMissedOpportunities(
  ...
  if (strategyId) params.set("strategy_id", strategyId);
  return apiFetch<MissedOpportunity[]>(
    `/api/diagnostics/missed?${params.toString()}`
  );
}

export async function fetchARSHeatmap(
  startDate?: string,
  endDate?: string,
): Promise<ARSHeatmap> {
  const params = new URLSearchParams();
  if (startDate) params.set("start_date", startDate);
  if (endDate) params.set("end_date", endDate);
  return apiFetch<ARSHeatmap>(
    `/api/diagnostics/ars?${params.toString()}`
  );
}
