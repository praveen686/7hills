...
def test_categorical_sample_and_expectation():
    from quantlaxmi.models.rl.core import Categorical
...
def test_gaussian_sample():
    from quantlaxmi.models.rl.core import Gaussian
...
def test_constant_distribution():
    from quantlaxmi.models.rl.core import Constant
    c = Constant(42)
    assert c.sample() == 42
...
def test_sampled_distribution():
    from quantlaxmi.models.rl.core import SampledDistribution
...
def test_non_terminal_and_terminal():
    from quantlaxmi.models.rl.core import NonTerminal, Terminal
    nt = NonTerminal(1)
    t = Terminal(2)
    assert nt.state == 1
    assert t.state == 2
...
def test_non_terminal_equality():
    from quantlaxmi.models.rl.core import NonTerminal
    a = NonTerminal('x')
    b = NonTerminal('x')
    assert a == b
...
def test_value_function_basics():
    from quantlaxmi.models.rl.core import ValueFunction, NonTerminal
    nt_a = NonTerminal('a')
    nt_b = NonTerminal('b')
...
def test_tabular_update():
    from quantlaxmi.models.rl.core import Tabular
...
def test_linear_fa_solve():
    from quantlaxmi.models.rl.core import LinearFunctionApprox
...
def test_linear_fa_update():
    from quantlaxmi.models.rl.core import LinearFunctionApprox, AdamGradient
    fa = LinearFunctionApprox(
        feature_functions=[lambda x: 1.0, lambda x: float(x)],
...
def test_dnn_approx_fit():
    from quantlaxmi.models.rl.core import DNNApprox, DNNSpec
    fa = DNNApprox(
...
def test_returns_computation():
    from quantlaxmi.models.rl.core import returns
...
def test_crra_utility_log():
    from quantlaxmi.models.rl.core import crra_utility
...
def test_cara_utility():
    from quantlaxmi.models.rl.core import cara_utility
...
def test_value_iteration_simple():
    ...
    """
    from quantlaxmi.models.rl.core import FiniteMarkovDecisionProcess, value_iteration
...
def test_moving_average():
    from quantlaxmi.models.rl.core import moving_average
...
def test_returns_from_rewards():
    from quantlaxmi.models.rl.core import returns_from_rewards
