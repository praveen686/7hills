...
def _run_s1(store, start: date, end: date, p: dict) -> dict:
    ...
    if variant == "options":
        ...
        from quantlaxmi.strategies.s1_vrp.density import run_multi_index_density_backtest
...
def _run_s3(store, start: date, end: date, p: dict) -> dict:
    ...
    from quantlaxmi.strategies.s9_momentum.data import available_dates, is_trading_day
...
def _run_s5(store, start: date, end: date, p: dict) -> dict:
    ...
    from quantlaxmi.strategies.s5_hawkes.research import _load_day_data, _backtest_feature
...
def _run_s6(store, start: date, end: date, p: dict) -> dict:
    ...
    from quantlaxmi.strategies.s6_multi_factor.research import _build_feature_matrix, _walk_forward_backtest
