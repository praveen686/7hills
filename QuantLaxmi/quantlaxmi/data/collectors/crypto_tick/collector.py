...
from quantlaxmi.data.collectors.crypto_tick.storage import BookTick, TickStore, TradeTick
from quantlaxmi.data.connectors.resilient_ws import ResilientWs, ResilientWsConfig
...
class TickCollector:
    ...
    async def _rescan_and_adjust(self) -> list[dict]:
        """Re-scan symbols and adjust subscriptions."""
    ...
    async def run(self, duration_sec: float | None = None) -> None:
        ...
        last_reconnect_count = self._ws.total_reconnects if self._ws else 0
        ...
        while self._running:
            ...
            # Detect WS reconnection -- re-subscribe all symbols
            current_reconnects = self._ws.total_reconnects
            if current_reconnects != last_reconnect_count:
                logger.info(
                    "WS reconnected (%d->%d), re-subscribing %d symbols",
                    last_reconnect_count, current_reconnects, len(self._subscribed),
                )
                resub = list(self._subscribed)
                self._subscribed.clear()
                await self.subscribe(resub)
