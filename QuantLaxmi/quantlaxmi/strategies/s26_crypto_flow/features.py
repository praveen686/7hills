...
class VPINState:
    ...
    CDF(dP / sigma_dP), where dP is the log-price change over the bar.
...
def kyles_lambda(
    ...
    """Rolling Kyle's Lambda: dP = lambda * signed_volume + epsilon.

    Signed volume = volume * sign(dP).
    lambda = Cov(dP, V_signed) / Var(V_signed).

    High lambda -> fragile market, large price impact per unit of flow.
    """
    n = len(prices)
