...
pub enum Commands {
    ...
    Paper {
        /// Path to configuration file (default: $QUANTLAXMI_CONFIG_ROOT/paper.toml or configs/paper.toml)
        #[arg(short, long)]
    ...
    match cli.command {
        ...
        Commands::Paper {
            ...
            let config_path = config.unwrap_or_else(|| {
                RunnerConfig::default_path(&ExecutionMode::Paper)
            });
            run_paper_mode(&config_path, headless, initial_capital).await
        }
        Commands::Live {
            config,
            initial_capital,
            ...
            let config_path = config.unwrap_or_else(|| {
                RunnerConfig::default_path(&ExecutionMode::Live)
            });
