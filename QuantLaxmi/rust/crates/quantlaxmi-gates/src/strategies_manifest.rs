...
mod tests {
    ...
    fn test_load_real_strategies_manifest() {
        ...
        let manifest_path = PathBuf::from(format!("{}/strategies_manifest.json", config_root));
    ...
    fn test_real_manifests_signal_binding() {
        ...
        let config_root = std::env::var("QUANTLAXMI_CONFIG_ROOT")
            .unwrap_or_else(|_| "configs".to_string());
        let signals_path = PathBuf::from(format!("{}/signals_manifest.json", config_root));
        let strategies_path = PathBuf::from(format!("{}/strategies_manifest.json", config_root));
