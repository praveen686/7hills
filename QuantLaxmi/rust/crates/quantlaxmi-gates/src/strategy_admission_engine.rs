...
impl StrategyAdmissionEngineConfig {
    ...
    ///
    /// Respects `QUANTLAXMI_CONFIG_ROOT` env var for locating config files.
    /// Falls back to `"configs"` if not set.
    pub fn new(session_id: impl Into<String>) -> Self {
        let config_root = std::env::var("QUANTLAXMI_CONFIG_ROOT")
            .unwrap_or_else(|_| "configs".to_string());
        Self {
            strategies_manifest_path: PathBuf::from(format!("{}/strategies_manifest.json", config_root)),
            signals_manifest_path: PathBuf::from(format!("{}/signals_manifest.json", config_root)),
            promotion_root: None,
...
///     .strategies_manifest("configs/strategies_manifest.json")
///     .signals_manifest("configs/signals_manifest.json")
