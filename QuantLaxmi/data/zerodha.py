...
def fetch_historical_chunked(
    ...
    """Fetch historical data in chunks to beat per-request date-range limits.

    Kite allows only ~60 days per request for minute intervals.
    This function splits the range into ``chunk_days`` windows,
    fetches each, and concatenates.
    """
    kite_interval = INTERVALS.get(interval)
    if kite_interval is None:
        raise ValueError(f"Unsupported interval '{interval}'. Use one of: {list(INTERVALS)}")
    ...
    now = datetime.now(timezone.utc)
    end_dt = pd.Timestamp(end).to_pydatetime() if end else now
    start_dt = pd.Timestamp(start).to_pydatetime() if start else end_dt - timedelta(days=60)
    ...
    while cursor < end_dt:
        ...
        try:
            ...
            logger.debug("Chunk %s→%s failed: %s", cursor, chunk_end, exc)
