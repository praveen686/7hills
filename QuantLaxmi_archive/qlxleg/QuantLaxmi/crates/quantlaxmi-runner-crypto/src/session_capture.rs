...
pub struct SessionCaptureConfig {
    ...
    /// Default price exponent (used when per-symbol not specified)
    pub price_exponent: i8,
    pub qty_exponent: i8,
    pub include_trades: bool,
    pub strict: bool,
    pub api_key: String,
    /// Optional per-symbol price exponents (overrides default).
    /// Use fetch_price_exponents() to populate automatically from Binance.
    pub price_exponents: Option<std::collections::HashMap<String, i8>>,
}
...
pub async fn capture_session(config: SessionCaptureConfig) -> Result<SessionCaptureStats> {
    ...
    for symbol in &config.symbols {
        ...
        // Use per-symbol exponent if available, else default
        let price_exp = config
            .price_exponents
            .as_ref()
            .and_then(|m| m.get(symbol).copied())
            .unwrap_or(config.price_exponent);
    ...
    manifest.diagnostics.regime_transitions.push(
        quantlaxmi_runner_common::artifact::RegimeTransition {
            ...
            features: serde_json::json!({
                ...
                "price_exponent": config.price_exponents
                    .as_ref()
                    .and_then(|m| m.get(symbol).copied())
                    .unwrap_or(config.price_exponent),
