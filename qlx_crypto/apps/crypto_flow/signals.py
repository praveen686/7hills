...
Signal A: Enhanced Carry
    ...
    Mathematical basis: Glosten-Milgrom adverse selection model — when
    the probability of informed trading is low, market makers are safe
    to provide liquidity (and we're safe to hold carry positions).
...
Signal B: Residual Carry
    ...
    Enter when residual > threshold, exit when residual normalizes.

Signal C: Cascade Direction
    During liquidation cascades (Hawkes intensity ratio > 3), trade
    in the direction of the cascade (momentum).  Cascade detection via
    Hawkes self-exciting process on trade arrivals.
...
def compute_regime(
    ...
    try:
        ...
        if len(timestamps) > 20:
            ...
            from apps.crypto_flow.features import HawkesState
...
def generate_residual_signals(
    ...
    # Entries
    slots = config.max_carry_positions - len(active_residual)
    if slots <= 0:
        return signals
