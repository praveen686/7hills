...
Proven (Signal A — Carry):
  ...
  Volume floor $10M. VPIN filter disabled (hourly kline VPIN is noise).
  90-day backtest: +4.18% avg, Sharpe 9.15, 9/12 profitable, max DD 1.34%.

With --with-ticks:
  Runs tick collector in background (aggTrade + bookTicker WebSocket).
  Feeds real tick-level VPIN, OFI, and Hawkes to the regime engine.
  Stores ticks to data/ticks/ for later analysis.

Usage:
    python3 -m apps.crypto_flow
    python3 -m apps.crypto_flow --with-ticks
    python3 -m apps.crypto_flow --scan-interval 300
    python3 -m apps.crypto_flow --once
"""
...
from apps.crypto_flow.signals import SignalConfig
from apps.crypto_flow.state import PortfolioState, compute_performance
...
async def run_with_ticks(args, config, state, state_path):
    ...
    The tick collector runs continuously in a background task, feeding
    real-time VPIN/OFI/Hawkes to the regime engine.
    """
    from apps.tick_collector.collector import CollectorConfig, TickCollector
    from apps.tick_collector.features import LiveRegimeTracker
    from apps.tick_collector.storage import TickStore, TickStoreConfig
