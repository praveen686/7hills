...
pub struct HydraStrategy {
    ...
    /// DIAGNOSTICS: Coherence samples for distribution analysis
    coherence_samples: VecDeque<f64>,
    /// DIAGNOSTICS: Coherence gate failures
    coherence_fail_count: u64,
    /// DIAGNOSTICS: Z-score gate failures
    zscore_fail_count: u64,
    /// DIAGNOSTICS: Cooldown gate failures
    cooldown_fail_count: u64,
    /// DIAGNOSTICS: Hysteresis gate failures
    hysteresis_fail_count: u64,
    /// DIAGNOSTICS: Cost basis samples
    cost_samples: VecDeque<f64>,
}

/// Strategy gate diagnostics for audit-grade reporting (Z-Phase spec section 9.4)
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct StrategyGateDiagnostics {
    // Tick-level stats
    pub ticks_seen: u64,
    pub ticks_with_full_d5: u64,
    pub ticks_synthetic: u64,

    // Signal flow stats
    pub candidate_signals: u64,
    pub orders_emitted: u64,
    pub fills_total: u64,

    // Gate pass/fail counters
    pub coherence_pass: u64,
    pub coherence_fail: u64,
    pub edge_pass: u64,
    pub edge_fail: u64,
    pub zscore_pass: u64,
    pub zscore_fail: u64,
    pub cooldown_fail: u64,
    pub hysteresis_fail: u64,

    // Distributions (min, median, p95)
    pub coherence_dist: (f64, f64, f64),
    pub edge_est_bps_dist: (f64, f64, f64),
    pub cost_bps_dist: (f64, f64, f64),

    // Configuration used
    pub config_coherence_min: f64,
    pub config_edge_hurdle: f64,
    pub config_zscore_threshold: f64,
    pub config_hysteresis: f64,
}

impl StrategyGateDiagnostics {
    pub fn new() -> Self {
        Self {
            ticks_seen: 0,
            ticks_with_full_d5: 0,
            ticks_synthetic: 0,
            candidate_signals: 0,
            orders_emitted: 0,
            fills_total: 0,
            ...
            cooldown_fail: 0,
            hysteresis_fail: 0,
            coherence_dist: (0.0, 0.0, 0.0),
            edge_est_bps_dist: (0.0, 0.0, 0.0),
            cost_bps_dist: (0.0, 0.0, 0.0),
            config_coherence_min: 0.0,
            config_edge_hurdle: 0.0,
            config_zscore_threshold: 0.0,
            config_hysteresis: 0.0,
        }
    }
}
...
impl HydraStrategy {
    ...
    pub fn with_config(config: HydraConfig) -> Self {
        ...
        Self {
            ...
            // DIAGNOSTICS: Gate tracking
            coherence_samples: VecDeque::with_capacity(10000),
            coherence_fail_count: 0,
            zscore_fail_count: 0,
            cooldown_fail_count: 0,
            hysteresis_fail_count: 0,
            cost_samples: VecDeque::with_capacity(10000),
        }
    }
    ...
    /// Get strategy gate diagnostics for audit-grade reporting
    pub fn get_diagnostics(&self) -> StrategyGateDiagnostics {
        // Helper to compute (min, median, p95) from samples
        fn compute_dist(samples: &VecDeque<f64>) -> (f64, f64, f64) {
            if samples.is_empty() {
                return (0.0, 0.0, 0.0);
            }
            ...
            (min, median, sorted[p95_idx])
        }

        StrategyGateDiagnostics {
            ticks_seen: self.tick_count,
            ticks_with_full_d5: self.tick_count, // Assume all ticks have D5 if they reach HYDRA
            ticks_synthetic: 0, // Tracked externally in pipeline
            candidate_signals: self.candidate_count,
            orders_emitted: self.passed_count,
            fills_total: 0, // Tracked externally
            coherence_pass: self.candidate_count - self.coherence_fail_count,
            coherence_fail: self.coherence_fail_count,
            edge_pass: self.passed_count,
            edge_fail: self.rejected_count,
            zscore_pass: self.candidate_count - self.zscore_fail_count - self.coherence_fail_count,
            zscore_fail: self.zscore_fail_count,
            cooldown_fail: self.cooldown_fail_count,
            hysteresis_fail: self.hysteresis_fail_count,
            coherence_dist: compute_dist(&self.coherence_samples),
            edge_est_bps_dist: compute_dist(&self.edge_samples),
            cost_bps_dist: compute_dist(&self.cost_samples),
            config_coherence_min: self.config.coherence_min,
            config_edge_hurdle: self.config.edge_hurdle_multiplier,
            config_zscore_threshold: self.config.zscore_action_threshold,
            config_hysteresis: self.config.position_hysteresis,
        }
    }
    ...
    fn execute_logic(&mut self, timestamp: i64, symbol: &str, price: f64) {
        ...
        // DIAGNOSTICS: Track coherence samples
        self.coherence_samples.push_back(coherence);
        if self.coherence_samples.len() > 10000 {
            self.coherence_samples.pop_front();
        }

        if coherence < self.config.coherence_min {
            self.coherence_fail_count += 1;
        ...
        // DIAGNOSTICS: Track cost samples
        self.cost_samples.push_back(total_cost);
        if self.cost_samples.len() > 10000 {
