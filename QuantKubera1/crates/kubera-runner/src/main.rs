    ...
    if let Some(cmd) = args.command {
        match cmd {
            ...
            Commands::RunZerodhaPipeline { quotes, strategy, out, commission_bps, capital } => {
                ...
                // Track tick-level diagnostics
                let mut ticks_synthetic = 0u64;
                let mut ticks_d5 = 0u64;
                for q in &quote_events {
                    if q.is_synthetic {
                        ticks_synthetic += 1;
                    }
                    if matches!(q.integrity_tier, kubera_options::replay::QuoteIntegrity::QuoteGradeD5) {
                        ticks_d5 += 1;
                    }
                }
                println!("  Tick integrity: {} D5, {} synthetic", ticks_d5, ticks_synthetic);
                ...
                // Write Strategy Gate Diagnostics (Z-Phase spec section 9.4)
                let diagnostics_dir = out_path.join("diagnostics");
                std::fs::create_dir_all(&diagnostics_dir)?;

                // Get HYDRA default config for reference
                let hydra_config = kubera_core::hydra::HydraConfig::default();

                let diagnostics_data = serde_json::json!({
                    "ticks_seen": quote_events.len(),
                    "ticks_with_full_d5": ticks_d5,
                    "ticks_synthetic": ticks_synthetic,
                    "candidate_signals": "N/A (internal to strategy)",
                    "orders_emitted": fills.len(),
                    "fills_total": fills.len(),
                    "config": {
                        "coherence_min": hydra_config.coherence_min,
                        "edge_hurdle_multiplier": hydra_config.edge_hurdle_multiplier,
                        "zscore_action_threshold": hydra_config.zscore_action_threshold,
                        "position_hysteresis": hydra_config.position_hysteresis,
                        "commission_bps": hydra_config.commission_bps,
                        "slippage_bps": hydra_config.slippage_bps
                    },
                    "diagnosis": if fills.is_empty() {
                        "0 fills generated. Possible causes: (1) coherence gate (experts not aligned), (2) edge gate (edge < hurdle × costs), (3) insufficient price movement in capture window. Recommend longer capture (2-3 hours minimum) or enable HYDRA debug logging."
                    } else {
                        "Strategy generated fills successfully."
                    },
                    "recommendation": if fills.is_empty() && ticks_d5 > 0 {
                        "Data quality is good (QuoteGradeD5). Issue is likely strategy selectivity. Run longer capture session covering market open/close volatility."
                    } else if ticks_synthetic > ticks_d5 {
                        "WARNING: More synthetic ticks than D5. Check Zerodha connection and depth_start offset."
                    } else {
                        "System operating normally."
                    }
                });
                let diagnostics_path = diagnostics_dir.join("strategy_gates.json");
                std::fs::write(&diagnostics_path, serde_json::to_string_pretty(&diagnostics_data)?)?;
                println!("  Diagnostics written to: {}", diagnostics_path.display());
